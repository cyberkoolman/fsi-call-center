@echo off
REM Script to create database schema in PostgreSQL (Windows batch version)
...
docker exec postgres psql -U postgres -d postgres -c "CREATE INDEX IF NOT EXISTS idx_customer_issues_customer_name ON customer_issues(customer_name); CREATE INDEX IF NOT EXISTS idx_customer_issues_call_date ON customer_issues(call_date); CREATE INDEX IF NOT EXISTS idx_customer_issues_resolve_status ON customer_issues(resolve_status);"

echo Database schema created successfully!

REM Display table structure
